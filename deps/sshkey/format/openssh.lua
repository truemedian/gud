...
local function get_keytype_impl(kt)
	for _, impl in ipairs(impls) do
		if impl.name == kt then return impl end
	end
...
local function decrypt_section(ciphername, kdfname, kdfoptions, data, passphrase)
	if ciphername == 'none' and kdfname == 'none' then return data end

	if not passphrase or passphrase == '' then return nil, 'decrypt_section: passphrase required' end
	...
	if #data % cipher_info.block_size ~= 0 then return nil, 'decrypt_section: invalid data length' end
...
local function load_private_openssh(data, passphrase)
	...
	if buf:read_bytes(15) ~= 'openssh-key-v1\x00' then return nil, 'load_private_openssh: bad magic' end
	...
	if nkeys > 1 then return nil, 'load_private_openssh: multiple keys' end

	if buf.loc ~= #buf.str + 1 then return nil, 'load_private_openssh: extraneous data' end
	...
	if not key.impl then return nil, 'load_private_openssh: unknown key type: ' .. key.kt end

	local pk_success, err
	pk_success, err = key.impl.deserialize_public(key, pubkey_buf)
	if not pk_success then return nil, err end
	...
	if not private_key_data then return nil, err end
	...
	if check1 ~= check2 then return nil, 'load_private_openssh: incorrect passphrase' end

	-- todo: handle certificates
	local privkey_keytype = private_key_buf:read_string()
	if privkey_keytype ~= key.kt then return nil, 'load_private_openssh: public and private key types do not match' end

	local sk_success
	sk_success, err = key.impl.deserialize_private(key, private_key_buf)
	if not sk_success then return nil, err end

	-- read comment and ensure padding is correct
	key.comment = private_key_buf:read_string()
	local padding = private_key_buf:left()
	for i = 1, #padding do
		if padding:byte(i) ~= i then return nil, 'load_private_openssh: invalid padding' end
	end

	do -- checksum to ensure private key and public key match
		local digest = openssl.digest.signInit(nil, key.sk)
		if not digest then return nil, 'load_private_openssh: allocation failed' end

		local signature, sig_err = digest:sign('signature-check')
		if not signature then return nil, 'load_private_openssh: ' .. sig_err end
...
local function load_private_other(data, passphrase)
	...
	if not pk then return nil, 'load_private_other: failed to extract public key' end

	local info = sk:parse()
	if not info then return nil, 'load_private_other: parse failed' end
...
---Decode an unarmored OpenSSH public key from the given data.
---@param data string
---@return sshkey.key|nil, string|nil
local function deserialize_public_openssh(data)
	local buf = buffer.read(data)
	local kt = buf:read_string()

	local impl = get_keytype_impl(kt)
	if not impl then return nil, 'deserialize_public_openssh: unknown key type: ' .. kt end

	local key = { kt = kt, impl = impl }
	local pk_success, pk_err = impl.deserialize_public(key, buf)
	if not pk_success then return nil, 'deserialize_public_openssh: ' .. pk_err end
...
local function load_public_openssh(data)
	...
	local kt = buf:read_string()
	if kt ~= prefix then return nil, 'load_public_openssh: key type mismatch' end

	local impl = get_keytype_impl(kt)
	if not impl then return nil, 'load_public_openssh: unknown key type: ' .. kt end

	local key = { kt = kt, impl = impl, comment = comment }
	local pk_success, pk_err = impl.deserialize_public(key, buf)
	if not pk_success then return nil, 'load_public_openssh: ' .. pk_err end
...
local function load_public_other(data)
	...
	if not pk then return nil, 'load_public_other: ' .. pk_err end

	local info = pk:parse()
	if not info then return nil, 'load_public_other: parse failed' end
...
local function save_public_openssh(key, comment)
	...
	if comment or key.comment then
		return prefix
			.. ' '
			.. openssl.base64(encoded, true, true)
			.. ' '
			.. ((comment or key.comment or ''):match('^%s*(.*)%s*$'))
	else
...
return {
	...
	deserialize_public_openssh = deserialize_public_openssh,
}
