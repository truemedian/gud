...
local function deserialize_public(key, buf)
	...
	if curve ~= keytype_to_curve(key.kt) then return nil, 'ecdsa.deserialize_public: curve mismatch' end

	local ec_name = keytype_to_nid(key.kt)
	if not ec_name then return nil, 'ecdsa.deserialize_public: unsupported curve' end
	...
	if not pk then return nil, 'ecdsa.deserialize_public: ' .. err end
...
local function deserialize_private(key, buf)
	...
	if not pk_success then return nil, pk_err end
	...
	if not sk then return nil, 'ecdsa.deserialize_private: ' .. err end
...
local function sign_raw(key, data)
	...
	if not digest then return nil, 'ecdsa.sign: allocation failed' end

	local signed = digest:sign(data)
	local container_tag, _, container_start, container_stop = asn1.get_object(signed)
	if container_tag ~= asn1.SEQUENCE then return nil, 'ecdsa.sign: malformed signature' end
...
local function verify_raw(key, signature, data)
	...
	if format ~= key.kt then return false, 'ecdsa.verify: format mismatch' end

	local algo = keytype_to_hashalgo(key.kt)
	local digest = openssl.digest.verifyInit(algo, key.pk)
	if not digest then return false, 'ecdsa.verify: allocation failed' end
