...
local function deserialize_public(key, buf)
	...
	if not pk then return nil, 'rsa.deserialize_public: ' .. err end
...
local function deserialize_private(key, buf)
	...
	if not sk then return nil, 'rsa.deserialize_private: ' .. err end
...
local function sign_raw(key, data)
	...
	if not digest then return nil, 'rsa.sign: allocation failed' end

	local signed = digest:sign(data)
	if not signed then return nil, 'rsa.sign: signing failed' end
...
local function verify_raw(key, signature, data)
	...
	if not digest then return false, 'rsa.verify: allocation failed' end
