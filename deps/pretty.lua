...
local default_theme = {
	...
	fail = "1;91", -- bold and bright red
	info = "1;34", -- bold and blue
	warn = "1;33", -- bold and orange
	debug = "1;35", -- bold and purple
	file = "1;97", -- bold and bright white
	code = "2;97", -- faint and bright white
}
...
function pretty.prettyPrint(...)
	...
	for i = 1, n - 1 do
		...
		assert(process.stdout:write(str))
...
---@param str string
---@param color_name string
---@param reset_name? boolean|string
---@param force_color? boolean
---@return string
function pretty.colorize(str, color_name, reset_name, force_color)
	if force_color or pretty.use_colors then
		if reset_name == false then
...
local single_quote2 = pretty.colorize("'", "quotes", false, true)
local double_quote1 = pretty.colorize('"', "quotes", "string", true)
local double_quote2 = pretty.colorize('"', "quotes", false, true)
local open_brace = pretty.colorize("{", "braces", false, true)
local clos_brace = pretty.colorize("}", "braces", false, true)
local open_bracket = pretty.colorize("[", "property", false, true)
local clos_bracket = pretty.colorize("]", "property", false, true)
local comma = pretty.colorize(", ", "sep", false, true)
local equals = pretty.colorize(" = ", "sep", false, true)
local zero = pretty.colorize("0", "number", false, true)
local star = pretty.colorize("*", "sep", false, true)
...
function pretty.dump(thing, depth, no_color)
	...
	local function preprocess(value, cur_depth)
		...
		elseif typ == "table" and not seen[value] and cur_depth < depth then
			...
			ret.name = pretty.colorize(tostring(value), typ, false, true)
			...
			for k, v in pairs(value) do
				...
				if type(k) == "string" and string.find(k, "^[_%a][_%a%d]*$") then
					pair.key = pretty.colorize(k, "property", false, true)
				else
			...
			return pretty.colorize(tostring(value), typ, false, true)
		end
	end
...
if process.stdout.is_tty then
	pretty.width = uv.tty_get_winsize(process.stdout.stream)
